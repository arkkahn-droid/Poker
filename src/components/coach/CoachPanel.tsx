...
export function CoachPanel() {
  ...
  const position = human ? getPositionLabel(human.seatIndex, state.dealerSeat, state.players.length) : ''
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-3 gap-px bg-gray-800 border-b border-gray-800">
        <StatCell label="Hands" value={progress.handsPlayed.toString()} />
        <StatCell label="VPIP" value={`${Math.round(progress.vpip * 100)}%`} />
        <StatCell label="Win%" value={progress.handsPlayed > 0 ? `${Math.round((progress.handsWon / progress.handsPlayed) * 100)}%` : '—'} />
      </div>
      ...
      <div className="border-t border-gray-800 p-3">
        <div className="text-xs text-gray-600 mb-2">Opponents at your table:</div>
        <OpponentLegend />
      </div>
    </div>
  )
...
function OpponentLegend() {
  const { aiPlayers } = useGameStore()
  return (
    <div className="space-y-1">
      {aiPlayers.map((p) => (
        <div key={p.id} className="flex items-center gap-2 text-xs">
          <span className="text-gray-400 w-16 truncate">{p.name}</span>
          <span className="text-gray-600">·</span>
          <span className="text-gray-500">{p.archetype.name}</span>
        </div>
      ))}
    </div>
  )
...
function getPositionLabel(seatIndex: number, dealerSeat: number, totalPlayers: number): string {
  return getPositionName(seatIndex, dealerSeat, totalPlayers)
}
